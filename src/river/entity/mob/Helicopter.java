...
/**
 * Clase de los Helicopteros
 * @author MauricioDaza
 */
public class Helicopter extends Entity implements Serializable {
    
    private Random random = new Random();
    private int facing = 0;

    /**
     * Constructor Parametrico de los Helicopteros
     * @param x
     * @param y
     * @param width
     * @param height
     ...
     * @param handler 
     */
    public Helicopter(int x, int y, int width, int height, boolean solid, Id id, Handler handler) {
    ...
    /**
     * Metodo que dibuja los Helicopteros
     * @param g 
     */
    public void render(Graphics g) {
        if(facing == 0){
            g.drawImage(Game.helicopter[1].getBufferedImage(), x, y, width, height, null);
        }
        if(facing == 1){
            g.drawImage(Game.helicopter[0].getBufferedImage(), x, y, width, height, null);
        }
    }

    /**
     * Metodo que actualiza los Helicopteros
     */
    public void tick() {
        x += vx;
        y += vy;
